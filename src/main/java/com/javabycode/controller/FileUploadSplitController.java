...
import com.javabycode.model.FileInfo;
import com.javabycode.service.WebUploader;

/**
 * https://github.com/callmeqingzhi/webuploaderSpringMVC
 * @author fhdone 2018年6月24日
 */
@Controller
@RequestMapping("uploadSplit")
public class FileUploadSplitController {

	private final static Logger log = LoggerFactory.getLogger(FileUploadSplitController.class);

	//文件上传目录，最好写在配置文件里
	private String uploadFolder="/Users/fhdone/Downloads/uploads";
	...
	public String fileUpload(String status, FileInfo info, @RequestParam(value = "file", required = false) MultipartFile file){
		...
		if(status == null){	//文件上传
			if(file != null && !file.isEmpty()){	//验证请求不会包含数据上传，所以避免NullPoint这里要检查一下file变量是否为null
				try {
					File target = webUploader.getReadySpace(info, this.uploadFolder);	//为上传的文件准备好对应的位置
					if(target == null){
						return "{\"status\": 0, \"message\": \"" + webUploader.getErrorMsg() + "\"}";
					}
					...
					if(info.getChunks() <= 0){
						if(!webUploader.saveMd52FileMap(info.getMd5(), target.getName())){
	...
	public String check(String status,FileInfo info){
		...
		if(status.equals("md5Check")){	//秒传验证
			...
			String path = webUploader.md5Check(info.getMd5());
			...
			//检查目标分片是否存在且完整
			if(webUploader.chunkCheck(this.uploadFolder + "/" + info.getName() + "/" + info.getChunkIndex(), Long.valueOf(info.getSize()))){
				return "{\"ifExist\": 1}";
			}else{
				return "{\"ifExist\": 0}";
			}
			...
			String path = webUploader.chunksMerge(info.getName(), info.getExt(), info.getChunks(), info.getMd5(), this.uploadFolder);
			if(path == null){
				return "{\"status\": 0, \"message\": \"" + webUploader.getErrorMsg() + "\"}";
			}
